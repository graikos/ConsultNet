...
class PaymentInfoFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        creditcard_button = ttk.Button(master=ChooseOptionFrame, text= "Credit Card" , style='Payment.TButton')
        style.configure('Payment2.TButton', background='white', foreground = 'black', font=('Montserrat', 8), relief='ridge', borderwidth =2 )
        style.map('TButton', background=[('active', '#eb6864')], foreground=[('active', 'black')])
        banktransfer_button = ttk.Button(master=ChooseOptionFrame, text= "Bank Transfer" , style='Payment2.TButton')
        ...
        price_button = ttk.Button(master=self, text= "Confirm payment" , style='Payment3.TButton')
